...
只返回JSON，不要其他内容。`;
      ...
      if (path.startsWith('/meetings/') && path.endsWith('/summary') && method === 'POST') {
        ...
        const { results: userQuestions } = await env.DB.prepare(`
          SELECT question, asker_name, created_at
          FROM user_questions
          WHERE meeting_id = ?
          ORDER BY created_at
        `).bind(meetingId).all();
        ...
        // 构建发言内容
        let discussionContent = statements.map((s, index) => 
          `${index + 1}. ${s.director_name}（${s.director_title}）：\n${s.content}`
        ).join('\n\n');

        // 如果有用户问题，也添加到分析内容中
        if (userQuestions.length > 0) {
          const questionContent = userQuestions.map((q, index) =>
            `用户提问${index + 1}（${q.asker_name}）：\n${q.question}`
          ).join('\n\n');
          discussionContent += '\n\n用户互动问题：\n\n' + questionContent;
        }
...
- 引用董事观点时要准确，体现其思维特色
- 只返回纯JSON格式，不要使用markdown代码块，不要添加代码块标记
